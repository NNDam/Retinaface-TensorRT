...
class TRT_Retinaface(object):
    ...
    def detect(self, batch_img, threshold: float = 0.6):
        ...
        t0 = time.time()
        result = self.postprocess_batch(sorted_net_out, threshold, batch_size = batch_size)
        t1 = time.time()
        print('Post: ', t1-t0)
        return result
...
parser = argparse.ArgumentParser(description='Port Face Detector from ONNX to TensorRT-Runtime.')
...
TRT_FILE_PATH = args.model_path
print('[INFO] Load model')
trt_face = TRT_Retinaface(TRT_FILE_PATH, batch_size = batch_size)
trt_face.prepare()
...
tik = time.time()
list_det, list_landmarks = trt_face.detect(inp)
tok = time.time() - tik
...
print('[INFO] Port to TensorRT completed!')